...
### Backend CMS (Payload)
- **Runtime**: Node.js 20.
- **Database**: SQLite (file cục bộ qua adapter `@payloadcms/db-sqlite`, dễ chạy demo, có thể nâng cấp lên Postgres/MySQL sau).
...
### Công cụ hỗ trợ
- **Giám sát**: Sentry, Logtail, Vercel Observability, Healthchecks. Repo đã cấu hình Sentry cho frontend (`sentry.client/server/edge.config.ts`) và CMS (`cms/monitoring/sentry.ts`).
...
- `payload.config.ts` đăng ký toàn bộ collection/globals, kết nối SQLite và plugin cloud storage.
...
- `src/lib/monitoring/sentry.ts` gom logic đọc biến môi trường, sample rate và trạng thái enable để dùng chung cho Next.js (client/server/edge).
- Frontend tự động khởi tạo Sentry thông qua `sentry.client.config.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`. Error boundary `app/[locale]/(public)/error.tsx` hiển thị mã sự cố (event id/digest) để hỗ trợ người dùng báo lỗi.
- CMS gọi `registerCmsMonitoring` trong `cms/monitoring/sentry.ts` từ `payload.config.ts` nhằm thêm request/error handler cho Express. Có thể mở rộng bằng cách gắn thêm transport tới Logtail/Datadog.
- Các biến môi trường `SENTRY_*` và `NEXT_PUBLIC_SENTRY_*` (tham khảo `.env.example`) điều chỉnh việc bật/tắt, sample trace và replay.
