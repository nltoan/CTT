...
Toàn bộ khung Next.js đã hoàn thiện các tuyến trang công khai (home, news, events, disciplines, galleries, people, partners, search) cho cả site chính lẫn sub-tenant. Từng block UI đều hỗ trợ tuỳ chỉnh theo theme/tenant và đã có dữ liệu seed để xem demo khi chạy cục bộ.

Các đầu việc còn lại trước khi bàn giao frontend production:

- **Kết nối Payload CMS thực tế**: thay thế các seed/mock bằng API Payload sau khi triển khai CMS (đã có schema trùng khớp). Dự kiến 1–2 ngày để dựng script đồng bộ và kiểm thử nội dung thật.
- **Hoàn thiện test & CI**: cài đặt dependencies để chạy `npm test`, bổ sung Playwright E2E mô phỏng luồng tạo tenant/trang. Dự kiến 1 ngày cấu hình + 1 ngày viết case chính.
- **Tinh chỉnh hiệu năng/A11y**: chạy Lighthouse, xử lý tối ưu ảnh (S3/CDN loader), kiểm tra WCAG (focus order, contrast). Khoảng 1 ngày cho vòng tối ưu.
- **Triển khai & cấu hình môi trường**: thiết lập biến môi trường Vercel, domain đa tenant, webhook ISR. Thời lượng tuỳ thuộc vào hạ tầng (ước tính 0.5–1 ngày).

Sau khi hoàn thành các bước trên, toàn bộ frontend đã sẵn sàng để mở preview và chuyển sang giai đoạn nhập nội dung thực tế.
...
- SEO tự động: metadata động theo trang/bài viết, alternates hreflang, JSON-LD (Organization, Navigation, Article, Event, ItemList), sitemap và robots.txt dựa trên dữ liệu seed; kèm bộ tạo Open Graph/Twitter image động theo tenant/locale.
